...
_COLOR_CHANNELS_KRAKENZ = {"external": 0b001, "screen": 0b1111}
...
class KrakenZ3(UsbDriver):
    ...
    def initialize(self, direct_access=False, **kwargs):
        ...
        def parse_led_info(msg):
            ...
            assert channel_count + 1 == len(self._color_channels) - (
        ...
        if len(mode) > 2:
            if mode[0].lower() == "config" and len(mode) > 2:
                if mode[1].lower() == "brightness":
                    assert int(mode[2]) >= 0 and int(mode[2]) <= 100, "Invalid brightness value"
                    self._write([0x30, 0x02, 0x01, int(mode[2]), 0x0, 0x0, 0x1, self.orientation])
                    return
                elif mode[1].lower() == "orientation":
                    assert int(mode[2]) >= 0 and int(mode[2]) <= 3, "Invalid orientation value"
            ...
            if mode[0].lower() == "static":
                ...
                loading_animation = open(os.path.join(pathlib.Path(__file__).parent.resolve(), 'loading.gif'), 'rb')
                loading_animation_data = loading_animation.read()
                self.send_data(loading_animation_data, [0x01, 0x0, 0x0, 0x0] + list(len(loading_animation_data).to_bytes(3, "little")))
                loading_animation.close()
                # send actual data requested
                data = self._prepare_gif_file(mode[1], self.orientation)
                self.send_data(data, [0x01, 0x0, 0x0, 0x0] + list(len(data).to_bytes(3, "little")))
                return
        elif len(mode) == 1:
            if mode[0].lower() == "liquid":
