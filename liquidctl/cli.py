...
def main():
    if sys.argv[1:3] == ['reserved', 'run-elevated']:
        logging.basicConfig(level=logging.DEBUG, format='[%(levelname)s] [run-elevated] %(name)s: %(message)s')
        status = liquidctl.elevate.run_elevated(sys.argv[3])
        sys.exit(status)
